...
Note that ActiveHelper is a generic helper framework not coupled to anything like Rails or Merb. Feel free to use clean helpers _everywhere_ in your projects (including Rails and friends)!

h2. Example

Let's use the bloody MVC-View example as we can find it in Rails or Merb (Sinatra, too?).
...
The exemplary _tag_ method took me days to implement. 
...
That's _a bit_ cleaner than blindly including 30 helper modules in another helper in another helper.
...
Obviously the view can invoke stuff from the FormHelper itself and also provided methods from its ancestors.

h3. Delegation as Multiple Inheritance

What if the _#form_tag_ method needs to access another method from another helper? In Rails, this would simply be
...
So we have to know _#url_for_ is located in the _UrlHelper_ and we even have to declare our helper @uses@ the other one.
That's a good thing for a) *code tidiness*, b) *good architecture* and c) *debugging*.

How does the _UrlHelper_ look like?
...
Next chance, who or what did create _@https_request?_ and where does it live?

Our helper bets on declaring its interface, again! This time we define what comes in. 
...
It tells us what it @needs@ and that's all for it. Any call to _#https_request?_ (that's a /method/) is strictly delegated back to the target, that's the view and the view alone has to care.

That's what happens in productive mode.
...
h2. Concepts
* no raw @ivar access in helpers (although they could, when defined in the constructor)
* model dependencies with OOP strategies: inheritance! AND #needs ?????

helpers are usually pushed into _instances_, if you don't want that, go and have fun with modules. (or use delegation on class "layer" with #uses)
